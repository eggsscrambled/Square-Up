...
public class WeaponAimController : NetworkBehaviour
{
    ...
    private void Fire(Vector2 direction, WeaponData data, Vector3 spawnPos)
    {
        ...
        // Get prefab settings
        var bulletSettings = data.bulletPrefab.GetComponent<NetworkedProjectile>();
        bool useGravity = bulletSettings != null ? bulletSettings.GetUseGravity() : false;
        float gravityScale = bulletSettings != null ? bulletSettings.GetGravityScale() : 1f;

        for (int i = 0; i < data.bulletAmount; i++)
        {
            // Fixed name to match the helper method below
            ...
            {
                if (obj.TryGetComponent<NetworkedProjectile>(out var proj))
                {
                    proj.Velocity = spreadDir * data.bulletSpeed;
                    proj.Owner = Object.InputAuthority;
                    proj.Damage = data.damage;
                    proj.KnockbackForce = data.knockbackForce;
                    proj.UseGravity = useGravity;
                    proj.GravityScale = gravityScale;
                    proj.LifeTime = TickTimer.CreateFromSeconds(runner, data.bulletLifetime);
                }
            });
        }
    }

    // Helper method name fixed here
